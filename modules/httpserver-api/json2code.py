...
def print_h_file_headers(f, name):
    print_copyrights(f)
    fprint(f, f"#ifndef __JSON_AUTO_GENERATED_{name}")
    fprint(f, f"#define __JSON_AUTO_GENERATED_{name}" + "\n")


def clean_param(param):
    return match[1] if (match := re.match(r"(^[^\}]+)\s*}", param)) else param
        ...
        for i, item in enumerate(data["apis"]):
            if "operations" in item:
                try:
                    ...
                    raise Exception(f"{value.message} in apis[{str(i)}] path {path}")
        fprint(cfile, "}")
...
def is_model_valid(name, model):
    ...
    for var in properties:
        ...
        if type == "array":
            type = getitem(
                getitem(properties[var], "items", f"{name}:{var}"),
                "type",
                f"{name}:{var}:items",
            )
...
def create_h_file(data, hfile_name, api_name, init_method):
    with open(f"{config.outdir}/{hfile_name}", "w") as hfile:
        ...
        if "models" in data:
            ...
            for model_name in models_order:
                ...
                fprint(hfile, '}')
                fprint(hfile, model_name, f'(const {model_name}' + ' & e) {')
                fprint(hfile, '  register_params();')
        ...
        fprint(hfile, f"{init_method}(const std::string& description);")
...
def parse_file(param, combined):
    ...
    try:
        ...
        hfile_name = f"{base_file_name}.hh"
        api_name = base_file_name.replace('.', '_')
        base_api = base_file_name.replace('.json', '')
        init_method = f"void {api_name}_init_path"
        trace_verbose("creating ", hfile_name)

        cfile_name = f"{config.outdir}/{base_file_name}.cc"
        if (combined):
