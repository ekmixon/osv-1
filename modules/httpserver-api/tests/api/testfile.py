...
class testfile(basetest.Basetest):
    ...
    def test_put_file_cmd(self):
        ...
        hosts = self.curl(f"{path}/etc/hosts1?op=GETFILESTATUS")
        self.assertEqual(hosts["type"], "FILE")
        self.curl(
        ...
        hosts = self.curl(f"{path}/etc/hosts2?op=GETFILESTATUS")
        self.assertEqual(hosts["type"], "FILE")
        self.assertHttpError(f"{path}/etc/hosts1?op=GETFILESTATUS")
        self.curl(f"{path}/etc/hosts2?op=DELETE", method='DELETE')
        self.assertHttpError(f"{path}/etc/hosts2?op=GETFILESTATUS")

    def make_temp_file(self):
        with open('temp-test-file.txt', 'w') as f:
    ...
    def test_file_upload(self):
        ...
        subprocess.check_call(
            self.build_curl_cmd(
                f"-F filedata=@temp-test-file.txt {self.get_url(target)}"
            ).split()
        )

        hosts = self.curl(f"{target}?op=GETFILESTATUS")
        self.assertEqual(hosts["type"], "FILE")
        os.remove('temp-test-file.txt')
        subprocess.check_call(
            self.build_wget_cmd(
                f"-O tmp-test-dwnld.txt {self.get_url(target)}?op=GET "
            ).split()
        )
