...
if 'openjdk version "1.8.0' not in java_version:
...
usr_files.add(jdk_path).to('/usr/lib/jvm/java').include('jre/**').exclude(
    'jre/lib/security/cacerts'
).exclude(f'jre/lib/{arch_dir}/*audio*').exclude(
    f'jre/lib/{arch_dir}/*sound*'
).exclude(
    ''
).exclude(
    'jre/man/**'
